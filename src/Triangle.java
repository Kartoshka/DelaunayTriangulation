...
boolean isInCircumCircle(Point target)
{
  ...
  boolean circumCircle (Point target) {
	  float x1 =(float)this.p1.x; float y1 =(float)this.p1.y;
	    float x2 =(float)this.p2.x; float y2=(float)this.p2.y;
	    float x3=(float)this.p3.x; float y3=(float)this.p3.y;
	    float xp =(float) target.x; float yp=(float)target.y;
	    float xc, yc;
		float y1y2 = Math.abs(y1 - y2);
		float y2y3 = Math.abs(y2 - y3);
		if (y1y2 < EPSILON) {
			if (y2y3 < EPSILON) return false;
